...
public class ForgetPsswordStep1Fragment extends Fragment {
    ...
    public void RestaurantForgetPassword()
    {
        String RestaurantMai=ForgetPasswordStep1FrMailEt.getEditText().getText().toString().trim();
        ...
        apiService.RestaurantForgetPassword(RestaurantMai).enqueue(new Callback<UserResetPassword>() {
            @Override
            public void onResponse(Call<UserResetPassword> call, Response<UserResetPassword> response) {
                ...
                try {
                  if(response.body().getStatus().equals(1))
                  {
                      ...
                      HelperMethod.replace(forgetPasswordStep2Fragment,getFragmentManager(),R.id.SellNavigation_RootLayout);
                  }
                  else
                  {
