...
public class CurrentRequests extends Fragment {
    ...
    RecyclerView UserCurrentRequestsFrRv;
    List<UserRequestsData> UserRequestList=new ArrayList<>();
    ...
    public View onCreateView(LayoutInflater inflater, final ViewGroup container,
        ...
        UserCurrentRequestsFrRv.setLayoutManager(linearLayoutManager);
        OnEndless onEndless=new OnEndless(linearLayoutManager,1) {
            ...
            public void onLoadMore(int current_page) {
                ...
                if(current_page<=max)
                {
                    GetUserRequests(current_page);
                }
            }
        };
        UserCurrentRequestsFrRv.addOnScrollListener(onEndless);
        adapter=new UserRequestsRvAdapter(getContext(),UserRequestList,false);
        UserCurrentRequestsFrRv.setAdapter(adapter);
        GetUserRequests(1);
        return view;
    }
    ...
    public void GetUserRequests(int pagenumber)
    {
        ...
        apiService.GetUserRequests(apitoken,"current",pagenumber).enqueue(new Callback<UserRequests>() {
            @Override
                ...
                try {
                    ...
                    if(response.body().getStatus().equals(1)) {
                        if (response.body().getData().getData().size() == 0) {
                            ...
                            UserRequestList.addAll(response.body().getData().getData());
